...
$storageAccountName = "mystorage" + $uniqueId
az storage account create --name $storageAccountName  --resource-group $resourceGroup --location $location --sku Standard_LRS --kind StorageV2 --hns true
$storageAccountId = (az storage account show -g $resourceGroup -n $storageAccountName --query id -o tsv)
az storage container create --name schema --account-name $storageAccountName --only-show-errors
az extension add --upgrade --name azure-iot-ops
az iot ops schema registry create  --subscription $subscriptionId -g $resourceGroup -n jumpstartschreg --registry-namespace sacns --sa-resource-id $storageAccountId --sa-container schemas
$schemaRegistryId = (az iot ops schema registry show  --subscription $subscriptionId -g $resourceGroup -n jumpstartschreg --query id -o tsv)
az config set extension.use_dynamic_install=yes_without_prompt
$arcClusterName = (az connectedk8s list -g $resourceGroup --query '[0].name' --output tsv)

do {
    az iot ops init --subscription $subscriptionId -g $resourceGroup --cluster $arcClusterName  --only-show-errors
    az iot ops create  --subscription $subscriptionId -g $resourceGroup --cluster $arcClusterName --custom-location $arcClusterName-ledge-cl -n $arcClusterName-ledge-ops-instance --sr-resource-id $schemaRegistryId --broker-frontend-replicas 1  --broker-frontend-workers 1  --broker-backend-part 1  --broker-backend-workers 1  --broker-backend-rf 2  --broker-mem-profile Low  --only-show-errors
    if ($? -eq $false) {
        $aioStatus = "notDeployed"
        Write-Host "`n"
